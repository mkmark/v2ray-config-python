...
class AckList:
    ...
    next_flush: Optional[list[int]] = field(default_factory=list[int])
    flush_candidates: Optional[list[int]] = field(default_factory=list[int])
...
class ReceivingWorker(RWMutex):
    ...
    left_over: Optional[MultiBuffer] = field(default_factory=MultiBuffer)
