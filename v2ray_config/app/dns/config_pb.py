...
from v2ray_config.app.dns.fakedns.fakedns_pb import FakeDnsPoolMulti
from v2ray_config.app.router.routercommon.common_pb import GeoIP, GeoSite
from v2ray_config.common.net.destination_pb import Endpoint
...
class NameServer:
    ...
    prioritized_domain: Optional[list[NameServer_PriorityDomain]] = field(
        default_factory=list[NameServer_PriorityDomain]
    )
    geoip: Optional[list[GeoIP]] = field(default_factory=list[GeoIP])
    original_rules: Optional[list[NameServer_OriginalRule]] = field(
        default_factory=list[NameServer_OriginalRule]
    )
...
class SimplifiedNameServer:
    ...
    tag: Optional[str] = None
    prioritized_domain: Optional[list[SimplifiedNameServer_PriorityDomain]] = field(
        default_factory=list[SimplifiedNameServer_PriorityDomain]
    )
    geoip: Optional[list[GeoIP]] = field(default_factory=list[GeoIP])
    original_rules: Optional[list[SimplifiedNameServer_OriginalRule]] = field(
        default_factory=list[SimplifiedNameServer_OriginalRule]
    )
    fake_dns: Optional[FakeDnsPoolMulti] = field(default_factory=FakeDnsPoolMulti)
    skipFallback: Optional[bool] = None
    query_strategy: Optional[QueryStrategy] = field(default_factory=QueryStrategy)
...
class SimplifiedConfig:
    name_server: Optional[list[SimplifiedNameServer]] = field(
        default_factory=list[SimplifiedNameServer]
    )
    client_ip: Optional[str] = None
    static_hosts: Optional[list[SimplifiedHostMapping]] = field(
        default_factory=list[SimplifiedHostMapping]
    )
    fake_dns: Optional[FakeDnsPoolMulti] = field(default_factory=FakeDnsPoolMulti)
    tag: Optional[str] = None
    domain_matcher: Optional[str] = None
